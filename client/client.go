...
type Client struct {
	// 消息编码器，用来序列化将要发送出去的请求，以及反序列化接收到的响应
	cc  codec.Codec
	opt *codec.Option
	// 发送互斥锁,为了保证消息的有序发送
	sendMutex sync.Mutex // protect following
	// 消息头部
	header   codec.Header
...
// registerCall add call into pending and update the seq
...
// removeCall accroding seq to remove the pending and return Call
...
// terminateCalls invoked when server happend error then update client.shutdowm to true
